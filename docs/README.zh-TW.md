...
| **Coding Service Tool API** — `POST /api/v1/tools/*` (list_repos / inspect_repo / submit_coding_task / check_task_status / get_task_result / cancel_task),給上游 LLM agent 透過 function calling 呼叫;Phase B 加上細粒度 stage 翻譯與結構化 TaskResult | [設計文件](design/coding-service-tool-api.md) |
...
│   ├── test_tool_api.py                # 15 tests (Phase A + Phase B coding service)
