...
| `POST` | `/api/v1/tools/resolve_human_block` | 解封 agent 透過 `[HUMAN_REQUIRED]` escalate 的 task (記 hint + force-retry) |
| `POST` | `/api/v1/tools/list_tasks` | 列出 Tool API task;可選 `parent_task_id` + `status` 過濾 |
| `POST` | `/api/v1/tools/get_workflow_result` | 把一個 parent + 它的直接 children 合併成單一 rollup (D5) |
...
- **`[HUMAN_REQUIRED] <reason>` 升級標記** (D3):任何 runner
  (echo / opencode / anthropic_api / claude_cli) 都可以在 turn
  結束前用 `[HUMAN_REQUIRED] <一句話原因>` 單獨一行 escalate —
  orchestrator 在 release 時掃描偵測,把該 attempt 標成
  `terminal_reason=needs_human` 而不是 `agent_finished`。標記在
  fenced code block(`` ``` `` / `~~~`)裡的會被忽略,避免 agent
  引用 source / docs 時誤觸發。Attempt status 變成
  `blocked_for_human`,parent 之下所有 pending children 都會在 dispatch
  gate 凍結 (跟 parent-failure 同樣語意),直到解封。
- **`resolve_human_block`**:operator 回應端點。Body
  `{task_id, resolution_hint, author?}` 把 resolution 寫成 hint(audit
...
- **`get_workflow_result`** (D5):把一個 parent task + 它的**直接
  children**(走一層 — Path A 與 D2b 拆出來的都是 flat 子圖)合併成
  單一 rollup。Body 收 `{task_id}`(parent),回傳 `parent` +
  `children: [...]` + `aggregate: {cost_usd, duration_ms, files_changed
  (去重,同一 path 留最後一次 change)、blockers、follow_ups、counts
  (per-status 計數)}` 加上 rollup `status`。優先序:
  `blocked_for_human` > `failed` > `running`(含 pending) >
  `cancelled`(全 cancelled 才算) > `done`(全 done 才算)。
  支援 partial rollup,client 可以一直 poll 同一個 endpoint 直到
  rolled-up `status` 進入 terminal,不必跨 endpoint 拼。`task_id` 不
  存在才回 404。
