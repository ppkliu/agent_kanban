...
| Prometheus `/metrics` — 9 zero-dependency counters scrapeable by Prometheus/Grafana | ✅ | this batch | `curl http://localhost:17957/metrics` |
...
| Phase D1 subtask graph — `parent_task_id` + `depends_on` on `submit_coding_task`; parent-failure gate at dispatch; `list_tasks` endpoint for walking child graphs | ✅ | this batch | `pytest tests/test_tool_api.py tests/test_orchestrator.py -k "parent or depends or list_tasks"` |
| Phase D2a decomposition Path A — `subtasks=[...]` fans out to 1 parent + N children; sibling indices translated to real task_ids; one idempotency_key + trace_id covers the whole batch | ✅ | this batch | `pytest tests/test_tool_api.py -k subtask` |
...
| Phase D4 CHECKPOINT mid-flight progress — agent emits `[CHECKPOINT] <msg>` or `[CHECKPOINT step=2/5] <msg>` inside any assistant message; orchestrator scans MESSAGE_DELTA text and fans synthetic `checkpoint` events through the bridge; events tab + kanban card show progress live (fenced markers ignored) | ✅ | this batch | `pytest tests/test_checkpoint.py tests/test_orchestrator.py -k checkpoint` |
| Phase D5 cross-task rollup — new `get_workflow_result` Tool API endpoint aggregates a parent + its direct children (Path A or D2b subgraphs) into one response with per-status counts, summed cost, wall-clock duration, deduped files / blockers / follow-ups; status precedence `blocked_for_human > failed > running > cancelled > done`; partial rollups returned so callers poll one endpoint instead of two | ✅ | this batch | `pytest tests/test_tool_api.py -k workflow_result` |
| Self-contained test suite (no LLM / no GitHub required) | ✅ | 294 tests | `.venv/bin/python -m pytest` |
...
| Prompt injection defence (Phase 2 token-level + LLM-judge) | Medium | Phase 1 framing **shipped** (see checklist above); per-runner token isolation and LLM-judge layers remain as defence-in-depth upgrades for high-sensitivity deployments. |
