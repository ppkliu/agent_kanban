...
Every open WS also receives a **heartbeat** message at a dynamic
cadence — this is the primary health signal browser SPAs consume (not
`/healthz`):
...
Cadence: **15s** when any attempt is CLAIMED or RUNNING (active mode);
**150s** when no attempt is in flight (idle mode). `idle` reports the
selected mode and `next_heartbeat_after_s` tells the SPA when to
expect the next one — clients self-calibrate stale thresholds at
`2 × next_heartbeat_after_s` (degraded) and `6 × next_heartbeat_after_s`
(unhealthy), so idle mode does not trip a warning sized for active
mode. `orchestrator_ticks` is a monotonic counter incremented on every
`Orchestrator.tick()` — a stuck main loop (process alive, ticks
frozen) is detectable by watching it stop growing even when the WS
itself stays open. The dashboard TopBar status chip implements
exactly these heuristics.
...
|---|---|---|---|
| `GET` | `/healthz` | **Liveness backstop** — returns `{"ok": true}`. Used by docker-compose / k8s probes only; the primary "backend healthy" signal for browser SPAs is the WebSocket heartbeat (see below). Default docker healthcheck interval: 900s (15 min). | ❌ no auth (intentional — lets `docker compose` healthcheck poll without a token) |
