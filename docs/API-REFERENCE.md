...
Every ~15s, every open WS also receives a **heartbeat** message —
this is the primary health signal browser SPAs consume (not `/healthz`):

```json
{"type": "heartbeat", "server_time": "2026-05-19T14:00:00+00:00", "orchestrator_ticks": 1234}
```

`orchestrator_ticks` is a monotonic counter incremented on every
`Orchestrator.tick()`. A SPA can detect a stuck main loop (process
alive, ticks frozen) by watching for the counter to stop growing, even
when the WS itself stays open. Treat the WS as **healthy** if a
heartbeat arrived in the last 30s, **degraded** between 30–90s old, and
**unhealthy** above 90s — the dashboard TopBar status chip uses exactly
these thresholds.
...
|---|---|---|---|
| `GET` | `/healthz` | **Liveness backstop** — returns `{"ok": true}`. Used by docker-compose / k8s probes only; the primary "backend healthy" signal for browser SPAs is the WebSocket heartbeat (see below). Default docker healthcheck interval: 60s. | ❌ no auth (intentional — lets `docker compose` healthcheck poll without a token) |
